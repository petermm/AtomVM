...
static struct EtsTable *get_table(
    ...
    int32_t process_id,
    TableAccess access);
static EtsStatus add_table(Ets *ets, struct EtsTable *table);
...
static EtsStatus insert_one(
    struct EtsTable *table,
    term tuple,
    bool as_new,
    Context *ctx);
static EtsStatus insert_many(
    struct EtsTable *table,
    term tuples,
    bool as_new,
    Context *ctx);
static EtsStatus lookup_select_maybe_gc(
    struct EtsTable *table,
    term key,
    size_t index,
    size_t num_roots,
    ...
    term *ret,
    Context *ctx);
static EtsStatus lookup_or_default(
    struct EtsTable *table,
    term key,
    term default_tuple,
    Heap *ret_heap,
    term *ret,
    Context *ctx);
static EtsStatus apply_spec(term tuple, term spec, size_t key_index);
static EtsStatus apply_op(term tuple, term opt, avm_int_t *ret, size_t key_index);
...
EtsStatus ets_create_table_maybe_gc(
    term name,
    bool named,
    EtsTableType type,
    EtsTableAccess access,
    ...
    Context *ctx)
{
    ...
    EtsStatus result = add_table(&ctx->global->ets, table);
...
EtsStatus ets_lookup_maybe_gc(term name_or_ref, term key, term *ret, Context *ctx)
{
    ...
    EtsStatus result = lookup_select_maybe_gc(table, key, ETS_WHOLE_TUPLE, 0, NULL, ret, ctx);
...
EtsStatus ets_lookup_element_maybe_gc(term name_or_ref, term key, size_t index, term *ret, Context *ctx)
{
    ...
    EtsStatus result = lookup_select_maybe_gc(table, key, index, 0, NULL, ret, ctx);
    ...
    Context *ctx)
{
    ...
    Heap insert_heap;
    term insert_tuple;
    EtsStatus result = lookup_or_default(table, key, default_tuple, &insert_heap, &insert_tuple, ctx);
    if (result != EtsOk) {
        SMP_UNLOCK(table);
        return result;
    }
...
EtsStatus ets_update_counter_maybe_gc(
    term name_or_ref,
    term key,
    term op,
    term default_tuple,
...
EtsStatus ets_take_maybe_gc(term name_or_ref, term key, term *ret, Context *ctx)
{
    struct EtsTable *table = get_table(
        &ctx->global->ets,
        name_or_ref,
    ...
    EtsStatus result = lookup_select_maybe_gc(table, key, ETS_WHOLE_TUPLE, 1, &key, ret, ctx);
...
EtsStatus ets_delete(term name_or_ref, term key, Context *ctx)
{
    ...
    EtsStatus result = ets_multimap_remove(table->multimap, key, ctx->global);
...
EtsStatus ets_delete_table(term name_or_ref, Context *ctx)
{
...
EtsStatus ets_delete_object(term name_or_ref, term tuple, Context *ctx)
{
    struct EtsTable *table = get_table(
        &ctx->global->ets,
        name_or_ref,
    ...
    EtsStatus result = ets_multimap_remove_tuple(table->multimap, tuple, ctx->global);
    ...
    Context *ctx)
{
    ...
    *ret = term_nil();
    ...
    size_t count;
    EtsStatus result = ets_multimap_lookup(table->multimap, key, &tuples, &count, ctx->global);
    if (UNLIKELY(result == EtsAllocationError)) {
        return EtsAllocationError;
    }
    ...
    Context *ctx)
{
    ...
    term *tuple = NULL;
    size_t count;
    EtsStatus result = ets_multimap_lookup(table->multimap, key, &tuple, &count, ctx->global);
    if (result != EtsOk) {
        return result;
    }
...
static EtsStatus apply_op(term tuple, term op, avm_int_t *ret, size_t key_index)
{
