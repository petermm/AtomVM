...
{
    size_t error_size = PORT_REPLY_SIZE + TUPLE_SIZE(2) + TUPLE_SIZE(2) + TERM_BINARY_HEAP_SIZE(reason);
...
static void send_scan_results(struct ScanClientData *data)
{
    ...
    esp_err_t err;
    wifi_ap_record_t *ap_records = calloc(num_results, sizeof(wifi_ap_record_t));
    if (IS_NULL_PTR(ap_records)) {
        BEGIN_WITH_STACK_HEAP(PORT_REPLY_SIZE + TUPLE_SIZE(2) + TUPLE_SIZE(2), heap);
        term error = port_heap_create_tuple2(&heap, ERROR_ATOM, OUT_OF_MEMORY_ATOM);
        term error_reply = port_heap_create_tuple2(&heap, make_atom(data->global, ATOM_STR("\xc", "scan_results")), error);
        term ref = term_from_ref_ticks(data->ref_ticks, &heap);
        term msg = port_heap_create_tuple2(&heap, ref, error_reply);
        port_send_message_from_task(data->global, term_from_local_process_id(data->owner_process_id), msg);
        END_WITH_STACK_HEAP(heap, data->global);
    ...
    if (UNLIKELY_NOT_ESP_OK(err)) {
        ...
        BEGIN_WITH_STACK_HEAP(PORT_REPLY_SIZE + TUPLE_SIZE(2) + TUPLE_SIZE(2) + error_len, heap);
        term reason = term_from_string((const uint8_t *) error, (uint16_t) error_len, &heap);
        term error_tuple = port_heap_create_tuple2(&heap, ERROR_ATOM, reason);
        term error_reply = port_heap_create_tuple2(&heap, make_atom(data->global, ATOM_STR("\xc", "scan_results")), error_tuple);
        term ref = term_from_ref_ticks(data->ref_ticks, &heap);
        term msg = port_heap_create_tuple2(&heap, ref, error_reply);
        port_send_message_from_task(data->global, term_from_local_process_id(data->owner_process_id), msg);
        END_WITH_STACK_HEAP(heap, data->global);
