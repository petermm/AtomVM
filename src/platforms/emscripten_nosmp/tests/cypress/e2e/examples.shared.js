...
function registerExamplesTests(paths) {
  describe(`${paths.label} examples homepage`, () => {
  ...
  describe(`${paths.label} hello world AVM`, () => {
    it("should output to console", () => {
      cy.visit(paths.helloWorldAvm, {
        onBeforeLoad(win) {
  ...
  describe(`${paths.label} hello world BEAM`, () => {
    it("should output to console", () => {
      cy.visit(paths.helloWorldBeam, {
        onBeforeLoad(win) {
          cy.stub(win.console, "log").as("consoleLog");
          cy.stub(win.console, "error").as("consoleError");
        },
  ...
  describe(`${paths.label} run script`, () => {
    it("should display three alerts and update counter", () => {
      cy.visit(paths.runScript, {
        onBeforeLoad(win) {
          cy.stub(win.console, "log").as("consoleLog");
        },
      });
  ...
  describe(`${paths.label} call & cast`, () => {
    it("should update counters when clicked", () => {
      cy.visit(paths.callCast);
  ...
  describe(`${paths.label} HTML5 Events`, () => {
    it("should work with key events", () => {
      cy.visit(paths.html5Events);
    ...
    it("should work with click events with coordinates", () => {
      ...
      cy.get("#event-1 td.event-map").should(($eventMap) => {
        ...
        expect(eventMapText)
          .to.match(/target_x => [23]/)
          .and.match(/target_y => [23]/);
      });
