...
-type scan_done_config() ::
    {scan_done, fun((scan_results() | {error, Reason :: term()}) -> term())}.
...
%% the callback will receive scan results or error tuples.
...
%% recommends not exceeding 1500 ms per-channel scan times or network connections may be lost, this is
...
%% </table>
%%
%% Optionally a callback may be configured for scan done events. In this case this function will return `ok'
%% immediately after a scan is successfully initiated, avoiding having the caller be blocked while waiting
%% for results. The callback function will receive the bare results tuple without being wrapped in an `ok'
%% tuple. In the event of an error while scanning the callback wil receive an `{error, Reason}' tuple.
%%
%% For convenience `network:wifi_scan/0' may be used to scan with default options.  The driver must already
%% be started, and not in the process of connecting to an access when this function is used. Only one scan
%% may be active at a time. If a second scan is initiated before the results from the first scan, the
%% first will be canceled and `{error, canceled}' will be returned to the caller.
%%
%% Note, if a long dwell time is used, the return time for this function can be considerably longer
%% than the default gen_server timeout, especially when performing a passive scan. Short dwell times
%% can easily miss networks, so applications need to be adjusted for their environment. Passive scans
...
%% @doc Equivalent to `wifi_scan/1' with default options from `sta_config/0'.
%% @end
-spec wifi_scan() ->
    ok
    | {ok, scan_results()}
...
wifi_scan() ->
    ...
    case proplists:get_value(sta, Config) of
        ...
        StaCfg ->
            ...
            Dwell = proplists:get_value(scan_dwell_ms, StaCfg, 120),
...
handle_call(
    {scan, ScanOpts}, From, #state{scan_receiver = SendTo, config = Config} = State
) ->
    case SendTo of
        undefined ->
            ok;
        {{Pid, _Tag} = Previous, _} when is_pid(Pid) ->
            gen_server:reply(Previous, {error, canceled});
        {Pid, _} when is_pid(Pid) ->
            Pid ! {scan_results, {error, canceled}};
        {Fun, _} when is_function(Fun, 1) ->
            spawn(fun() -> Fun({error, canceled}) end);
        _ ->
            ok
    end,
    ScanRef = make_ref(),
